...
<?php 
    ...
    $dataHj = $d->format('d-m-Y');
        ...
        for ($i=0; $i < count($reserva); $i++) { 
            if ($reserva[$i]['data_final'] == $dataHj) {
                echo "
            <tr>
                <td>".$reserva[$i]['id']."</td>
                ...
                <td><center><a href='opcoes-reserva.php?opcao=hj&id=".$reserva[$i]['id']."&idquarto=".$reserva[$i]['id_quarto']."&acao=delete'><i class='fa fa-trash' style='color:tomato;'></i></a></center></td>
            </tr>
            ";
            }
